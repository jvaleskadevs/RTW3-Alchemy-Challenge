...
import { Alert, Button, Col, InputNumber, Menu, Row, List, Divider } from "antd";
...
function App(props) {
  ...
  const [amountToStake, setAmountToStake] = useState(0.5);
  ...
  // ** keep track of a variable from the contract in the local React state:
  const claimPeriodLeft = useContractReader(readContracts, "Staker", "claimTimeLeft");
  ...
  */
  ...
  const onStakingETHChange = (value) => {
        ...
        <Switch>
          <Route exact path="/">
            ...
            <div style={{ padding: 8 }}>
			  <InputNumber
				defaultValue={0.5}
				formatter={(value) => `Ξ ${value}`.replace(/[^0-9\.Ξ]+/g, '')}
				parser={(value) => value.replace(/\$\s?|(,*)/g, '')}
				onChange={onStakingETHChange}
			  />
              <Button
                type={balanceStaked ? "success" : "primary"}
                onClick={() => {
                  tx(writeContracts.Staker.stake({ value: ethers.utils.parseEther(amountToStake.toString()) }));
                }}
              >
                🥩 Stake!
              </Button>
              <div>Ensure you have enough ETH or the program will fail!</div>
            </div>
            ...
            <div style={{ padding: 8 }}>
              <Button
                type={"default"}
                onClick={() => {
                  tx(writeContracts.Staker.recoverFunds());
                }}
              >
                 💵 Recover Funds
              </Button>
